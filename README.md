...
审核应用、审核模板及“通知群 + 审核员”方案通过共享审核配置接口保存；项目只选择一份审核配置及其中一个通知方案。登录异常模板、运维通知对象和星图账号仍属于项目，不应硬编码到仓库。
...
审核配置可被多个项目复用，并统一包含审核通知应用、卡片模板、审核结果字段以及多组“通知群 + 审核员”方案。项目切换绑定时，发送应用、模板、群聊和审核员会作为一个整体切换。

每个活动期次再配置直播/视频内容任务、业务多维表、审核表、手动登记表、追踪窗口和调度开关。自动工作流只运行启用且处于追踪窗口内的期次；不同项目受控并行，单个项目内部步骤保持顺序执行。一个项目失败只记录该项目错误，不会中止其他项目。

期次可以只配置直播或只配置视频；关闭 `morning_workflow_enabled`（状态接口对应
`morning_review_enabled`）时继续同步数据，但不发送审核通知。两个入口也接受
`audit_notice_enabled` 作为输入别名。

特殊期次不需要审核表时可省略 `audit_table_id`；服务以数据库 `NULL` 保存，并只跳过审核表及其
审核结果/通知统计流程，业务主表同步不受影响。
