...
- `periodic` 固定在北京时间 `13:00`、`18:00`；`morning` 在 `09:00`；`night` 在 `23:59:59`。不再提供按小时固定间隔配置。
...
- 跨实例工作流使用短事务数据库租约并定期续租，禁止用贯穿整个同步过程的长事务/advisory lock；续租失败时必须立即停止工作流，禁止在失去全局租约后继续执行。
- 运行台账必须通过独立数据库连接持续更新 `heartbeat_at`；孤儿恢复只能回收超过安全窗口未更新心跳的任务，禁止仅依据任务开始时间误杀仍在执行的长任务。
- 服务启动必须在 migration 后执行数据库自检：验证读写事务、北京时间时区和关键表，安全清理明确过期的应用 nonce/登录会话，再按心跳回收真正失联的工作流；禁止自动终止 PostgreSQL 中其他实例的事务。
- 工作流接口尽量支持可选 `activity_period_id`，用于只执行指定项目。
- `tracking_end_date=YYYY-MM-DD` 时，北京时间结束日 `23:59:59` 执行最后一次更新，次日不再拉取。
...
- 审核通知按项目分别发送，每个项目一张卡；项目绑定一份可共享的审核配置及该配置下的一项通知方案，审核人只读取所选通知方案中 `is_active=true` 的 auditor。
...
- 看板和业务查询默认要求飞书登录 JWT；默认环境应用登录拥有全项目权限，数据库中的非默认应用按 `xingtu_feishu_app_project_access` 实时限制可查看、可配置项目。`INTERNAL_API_TOKEN` 只能用于可信内部服务读取查询接口，不能获得管理权限；`MUTATION_API_TOKEN` 继续作为默认管理员兼容入口。
- 飞书 OAuth 登录只换取一次基础身份信息，服务端不保存 `user_access_token`；JWT 有效期 7 天且会话必须支持数据库撤销。`AUTH_REDIRECT_URIS` 使用完整地址精确白名单，禁止通配回调地址。
